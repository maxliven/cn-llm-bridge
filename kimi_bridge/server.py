...
专注场景：深度合成、长文本推理、跨模态综合分析。

启动方式:
  1. Claude Code 自动启动（通过 mcp.json 配置）
  2. 手动调试: python -m kimi_bridge.server
"""
...
class KimiClient:
    ...
    async def _post(
        self, path: str, json_data: dict, retries: int = 1
    ) -> dict:
        """HTTP POST 调用，带自动重试。"""
...
async def list_tools() -> list[Tool]:
    return [
        ...
        Tool(
            ...
            description=(
                "Kimi 多源综合工具。将多段分析结果"
                "（如多张图的 vision_analyze 输出）"
                "发送给 Kimi 进行深度综合和关联分析。"
                "适合 L2 级别的跨模态合成。"
            ),
            inputSchema={
                "type": "object",
