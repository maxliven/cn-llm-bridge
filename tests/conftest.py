...
def mock_env():
    ...
    os.environ.setdefault("BAILIAN_BASE_URL", "https://dashscope.aliyuncs.com/compatible-mode/v1")
    os.environ.setdefault("KIMI_BASE_URL", "https://api.moonshot.cn/v1")
...
def sample_vision_response():
    ...
    return {
        "choices": [{
            "message": {
                "content": '{"summary": "test summary", "details": "test details", "text_found": "sample text", "objects": ["button", "text field"]}'
            }
        }]
    }
...
def sample_kimi_response():
    ...
    return {
        "choices": [{
            "message": {
                "content": "Kimi analysis result: the code is well-structured."
            }
        }]
    }
